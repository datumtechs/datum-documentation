...
在原始数据传输至对应渠道后，用户若需要使得通过Datum存管在云服务、IPFS服务上的原始数据能被web3生态应用方检索到，但又不暴露所有原始数据信息，可以操作进行数据的Hash上链和元数据发布。
...
在使用存管数据的过程中，由于提供方的数据基于安全性考虑而做了加密处理并存储在私域，对于数据的使用方来说无法确认数据在使用过程中是否会被提供方篡改需要做一致性的校验。   我们可以通过上链的数据Hash对存管的原始数据的一致性进行校验，防止原始数据的篡改。


### 根据数据hash校验数据一致性

使用正在被使用原始数据的originId或者对应的元数据的metadataId，获取到存管在区块链上的数据Hash和被使用的数据做一致性校验处理。
...
|名称|类型|说明|是否必填|
|---|---|---|---|
|verifyType|int|表示verifyId的值类型，0：未知类型，1：originId，2： metadataId|Y|
|verifyId|string|原始数据上传后的originId或者对应的元数据metadataId,代表该原始数据在Datum中的唯一标识|Y|
|dataPath|string|被加载的原始数据缓存路径|Y|
...
```
"{
    "cloudType": 0    // 0: 使用Datum默认的方式存储； 1: 使用Datum提供的Google cloud存储； 2: 使用Datum提供的Aazon AWS存储；...
  }"
```
