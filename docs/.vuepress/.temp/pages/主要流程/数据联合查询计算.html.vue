<template><div><h1 id="数据联合查询计算" tabindex="-1"><a class="header-anchor" href="#数据联合查询计算" aria-hidden="true">#</a> 数据联合查询计算</h1>
<p>数据联合查询计算是Datum基于隐私增强技术面向两方、多方用户之间提供数据联合查询计算服务，解决传统数据共享的风险和担忧，为数据协作、数据流动提供技术服务能力支撑。Datum提供基于数据隐私保护的联合查询、联合统计分析、联合模型训练和预测的不同API供用户调用，通过集成PSI、MPC、HE等不同隐私增强技术，使用户无需过多了解不同的密码算法实现细节，降低了使用门槛。</p>
